...
def similarity_llm(query: str, csv_text: str) -> list:
    snap = sorted(Path("/home/share/SDSO/hf_cache/hub").glob(
        "models--microsoft--phi-4/snapshots/*"     # Choses model type
    ))[-1]
...
def query_mert(ranked: list, lookup: dict, context_lookup: dict) -> list:
    
    # Later, can adjust to use a shared cache for the SDSO project and remove the os.env statements below

    os.environ["HF_HOME"] = "/home/mason.dana/.cache/huggingface"  # ADJUST_LATER: private -> shared SDSO cache

    # Load MERT model and processor (m-a-p/MERT-v1-95M, the HF model)

    processor = Wav2Vec2FeatureExtractor.from_pretrained("m-a-p/MERT-v1-95M", trust_remote_code=True)
    mert = AutoModel.from_pretrained("m-a-p/MERT-v1-95M", trust_remote_code=True, device_map="auto")

    os.environ["HF_HOME"] = "/home/share/SDSO/hf_cache"     # Set Hugging Face cache directory back to shared cache for the SDSO project
    mert.eval()

    # Run the main loop to get embeddings per CLAP result
    ...
    for title, score in ranked:
        ...
        try:
            audio, _ = librosa.load(matches[0], sr=24000, mono=True)                            # Load in audio at 24kHz as a 1D array of audio samples (duration is not specified, so it will load the entire file, but only later use the first 60 seconds for feature extraction)
...
def query_output_llm(user_query: str, mert_results: list, csv_text: str) -> str:
    snap = sorted(Path("/home/share/SDSO/hf_cache/hub").glob(
        "models--microsoft--phi-4/snapshots/*"     # Choses model type
    ))[-1]
...
def main():
    ...
    top_results = similarity_llm(query, csv_text)
    final_results = query_mert(top_results, lookup, context_lookup)
    ...
    if final_results:
        ...
        print(query_output_llm(query, final_results, csv_text))
